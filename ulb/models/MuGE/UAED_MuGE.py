...
from .MuGE_utils import apply_frequency_mask, create_frequency_mask, crop, spectrum_noise
...
class ResizeConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, scale_factor=2, use_batchnorm=True):
        super().__init__()
...
class UnetPlusPlusDecoder(nn.Module):
    def __init__(
        ...
        self.upsample1 = ResizeConvBlock(decoder_channels[-1], decoder_channels[-1], scale_factor=2)
        self.upsample2 = ResizeConvBlock(decoder_channels[-1], decoder_channels[-1], scale_factor=2)
...
class Mymodel(nn.Module):
    def __init__(self, args, encoder_name="efficientnet-b7", encoder_weights="imagenet", classes=3):
    ...
    def forward(self, x):
        ...
        img_H, img_W = x.shape[2]*2, x.shape[3]*2
        ...
        for i in range(1, len(features)):
            ...
            mask = create_frequency_mask(a, b, radius=.4)
            low_freq = apply_frequency_mask(img_f1, mask, invert=False)
            high_freq = apply_frequency_mask(img_f1, mask, invert=True)
            low_freq_spatial = torch.fft.ifft2(torch.fft.ifftshift(low_freq, dim=(-2, -1))).real
            high_freq_spatial = torch.fft.ifft2(torch.fft.ifftshift(high_freq, dim=(-2, -1))).real


            # features[i] = torch.cat([features[i][:, :C, :, :] * (label_style).view(-1, 1, 1, 1), img_fft], 1)
            features[i] = torch.cat([img_f2, high_freq_spatial], 1)
